...
% noisy subgradient oracle.

close all;
clc;

load('SVM_dataset.mat');                % Load the generated dataset
data = all_data_shuffled;
...
num_realizations = 10;                   % No. of ensemble realizations for which the results are averaged
max_iters = 2000;                       % Maximum number of iterations in each realization
...
for realiz_ind = 1:1:num_realizations
    ...
    for k = 1:1:max_iters
        ...
        pred = sign(data(:,1:num_dims)*x_av);
        classification_error(1,k) = mean(pred.*data(:,num_dims+1) < 0);
...
for realiz_ind = 1:1:num_realizations
    ...
    for k = 1:1:max_iters
        ...
        q_ndo_grad = (1/R)*S*q_coeff;                          % Inverse transform from quantized coefficients
...
%%
% Top-K sparsification + Near-democratic + Vanilla Quantized Projected Subgradient Descent
...
semilogy(x_axis, obj_fnval_avg - cvx_optval);
hold on;
semilogy(x_axis, obj_fnval_avg_QPSGD - cvx_optval);
semilogy(x_axis, obj_fnval_avg_NDQPSGD - cvx_optval);
semilogy(x_axis, obj_fnval_avg_topK_QPSGD - cvx_optval);
semilogy(x_axis, obj_fnval_avg_topK_NDQPSGD - cvx_optval);
